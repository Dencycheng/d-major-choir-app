...
- 试用 API：`http://119.45.176.130:4173`。
...
当前 IP + HTTP 仅适合开发者工具、局域/临时调试或微信允许的特殊测试条件。正式体验版通常要求 HTTPS 且配置合法域名；备案完成后再统一切换。

```text
当前调试地址: http://119.45.176.130:4173
```
...
4. 确认腾讯云后端 `http://119.45.176.130:4173/api/health` 返回 `ok`。
