...
const { sectionLabel, isAudio, isVideo } = require("../../utils/format");
...
Page({
  ...
  setVideoRate(event) {
    const { id, rate } = event.currentTarget.dataset;
    const context = wx.createVideoContext(`video-${id}`, this);
    if (context.playbackRate) context.playbackRate(Number(rate));
    wx.showToast({ title: `已切换 ${rate}x`, icon: "none" });
  }
});
