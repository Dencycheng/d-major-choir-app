...
const defaultDbPath = path.join(root, "data", "dmajor.sqlite");
...
function readStore() {
  ...
  return {
    ...
    rolePermissions,
    members: db.prepare("SELECT id, user_id userId, name, nickname, avatar_file_id avatarFileId, avatar_url avatarUrl, mobile, email, section, role, status, note, voice_range voiceRange, attendance, managed_sections managedSections, created_at createdAt, updated_at updatedAt FROM members ORDER BY section, name").all().map(member => ({
      ...member,
...
function writeStore(store) {
  ...
  try {
    ...
    runMany(db, "INSERT INTO members (id, user_id, name, nickname, avatar_file_id, avatar_url, mobile, email, section, role, status, note, voice_range, attendance, managed_sections, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", store.members || [], member => [
      member.id,
      member.userId || null,
      member.name,
      member.nickname || "",
